  ...
  return (
    ...
    >
      <Swiper
        ...
        pagination={{
          clickable: true,
          el: ".custom-pagination", // 指定自定义分页容器
        }}
        className="h-[240px] rounded-2xl overflow-hidden bg-cover relative"
        style={{
          backgroundImage: "url(/src/assets/images/background-4.jpg)",
        }}
      ...
      >
        {data.map((item) => {
          return (
            <SwiperSlide className="rounded-2xl" key={item.cardId}>
              <div className="relative h-full p-6">
                <div className="absolute cursor-pointer right-4 top-4">
                  <Iconify
                    icon="solar:menu-dots-bold"
                    size={20}
                    className="opacity-[0.48] rotate-90"
                  />
                </div>

                <div className="h-[22px] mb-3 opacity-[0.48] font-semibold">
                  当前余额
                </div>
                <div className="h-[36px] mb-3 text-2xl font-bold flex gap-2 items-center">
                  {showNum ? (
                    <CountUp
                      end={item.num}
                      prefix="￥"
                  ...
                  <Iconify
                    icon={showNum ? "solar:eye-closed-bold" : "solar:eye-bold"}
                    size={20}
                    className="opacity-[0.48] cursor-pointer"
                    onClick={() => {
                      setShowNum(!showNum);
                    }}
                  />
                ...
                <div className="flex items-center justify-end gap-2 my-6">
                  <div className="px-[6px] w-9 h-6 bg-white flex items-center justify-center rounded-[4px]">
                    <svg
                ...
                <div className="flex gap-10 h-[50px]">
                  <div>
                    <div className="mb-2 text-xs opacity-[0.48]">卡包</div>
                    <span className="text-[16px] font-semibold">
                      {item.CardHolder}
                    </span>
                  </div>
                  <div>
                    <div className="mb-2 text-xs opacity-[0.48]">截止日期</div>
                    <span className="text-[16px] font-semibold">
                      {item.date}
                    </span>
                  </div>
        ...
        {/* 自定义分页容器 */}
        <div
          className="absolute custom-pagination"
